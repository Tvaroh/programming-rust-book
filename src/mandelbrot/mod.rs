...
pub fn run(args: Vec<String>, async_: bool) {
  if args.len() != 4 {
  ...
  if async_ { render_parallel(&mut pixels, bounds, upper_left, lower_right) }
  else { render(&mut pixels, bounds, upper_left, lower_right); }
...
fn escape_time(c: Complex<f64>, max_iterations: usize) -> Option<usize> {
...
fn write_image(filename: &str,
               pixels: &[u8],
               bounds: (usize, usize)) -> anyhow::Result<()> {
